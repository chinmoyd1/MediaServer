...
<a href="<?php echo '/mediaRIP/public_html/view_tv.php?id='.$Id.'&username='.$username.'&download';?>"> <div id='del'>
...
$Id = escape_string($_GET['id']);
     ...
     $query = query(" SELECT * FROM episodes WHERE eid = $Id");
   ...
   // echo $ex1;
    ...
    $query = query(" DELETE FROM episodes WHERE eid = $Id ");
    
    confirm($query);

    $det1='eplist.php?id='.$tvsid.'&username='.$username;
...
<?php
    $query = query(" SELECT * FROM episodes WHERE eid = " . escape_string($_GET['id']) . " ");
    
    confirm($query);
      while($row = fetch_array($query)):
      $video = $row['episode_location'];
      $tvsid = $row['id'];
      $ex = $row['episode_location'];
$explode = explode('/',$ex);
$ex1 = $explode[1];
...
<div id="container1">
     ...
     error_reporting(E_ALL); 
      ...
      $query = query(" SELECT * FROM tvSeries WHERE uniqueId = $tvsid " );
